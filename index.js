...
const phrases = [
  {
    name: "siara",
    texts: ["<target> nie chce mi się z Tobą gadać"],
    type: "phrase"
  },
  {
    name: "spadam",
  ...
  {
    name: "cr",
    texts: ["<!channel> Code-review samo się nie zrobi!!!"],
    type: "phrase"
  },
  {
    name: "pomylka",
  ...
  {
    name: "ufam",
    texts: ["Nikomu nie ufam, tylko tobie <target>."],
    type: "phrase"
  },
  {
    name: "lunch",
  ...
  {
    name: "pytanie",
    texts: ["Pytanie pierwsze. <target> Zawsze sikasz przez zapięty rozporek?"],
    type: "phrase"
  },
  {
    name: "idziemy",
    texts: ["No chodź <target>, noga! Idziemy na spacerek..."],
    type: "phrase"
  },
  {
    name: "cycki",
    texts: ["Cycki se usmaż <target>!"],
    type: "phrase"
  },
  {
    name: "dziendobry",
...
const processOutgoingResponse = request => {
  ...
  if (request.body.text.toLowerCase() === "help") {
    return `*Dostępne komendy:* ${phrases
      .map(phrase => phrase.name)
      .join(", ")}`;
  } else {
    try {
      const texts = phrases.find(
        item => item.name === request.body.text.toLowerCase()
      ).texts;
      ...
      // console.log(e);
      const text = phrasesHelp[Math.floor(Math.random() * phrasesHelp.length)];

      return text
        .replace("<target>", "<@" + request.body.user_id + ">")
        .replace("<command>", request.body.text);
    }
  }
};
  ...
  request.post(
    req.body.response_url,
    {
      json: {
        ...
        text: processOutgoingResponse(req)
      }
    },
    (error) => {
      if (error) {
